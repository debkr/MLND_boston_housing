...
# **Answer: **
# 
# Grid Search - also known as hyperparameter optimisation - is an exhaustive model optimisation technique used to identify the best parameter combination for a learning algorithm when evaluated against a specified scoring metric (ie the parameters which best minimise errors of bias and variance in the model).
# 
# It takes a sequence of parameter values (eg in our project maximum number of questions in the decision tree, max_depth = 1 to 10) inputted in the form of a dictionary, creates a grid of these parameter values, then systematically works through each grid value in turn, training and cross-validating the learning algorithm on the dataset for each one. Having completed the learning for all parameter values in turn, the Grid Search method identifies and returns the optimal parameter combination for the learning model.
# 
